...
class Agricola(Frame):
	...
	def collect_player_info(self):
		...
		for pi in range(self.num_players_var.get()):
			...
			self.players.append(Player(n=self.player_name_entries[pi].get(),f=food))
			self.player_tokens.append([[100,100*pi+100],[200,100*pi+100]])
	...
	def take_action(self,event):
		...
		if new_action:
			if new_action in self.played_actions:
				...
				self.player_tokens[self.current_player][self.players[self.current_player].get_family()-self.family_left[self.current_player]] = [(self.action_space_x[xi]+self.action_space_x[xi+1])/2,(self.action_space_y[yi]+self.action_space_y[yi+1])/2]
				#continue the round
	...
	def draw_board(self):
		...
		for pi in range(len(self.players)):
			...
			for fi in range(self.players[pi].get_family()):
				self.main_board_canvas.create_oval(self.player_tokens[pi][fi][0]-self.token_size,self.player_tokens[pi][fi][1]-self.token_size,self.player_tokens[pi][fi][0]+self.token_size,self.player_tokens[pi][fi][1]+self.token_size,fill='green')
