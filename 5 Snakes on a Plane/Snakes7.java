...
//    static long update;
    ...
    public static double check2 (int i1, int i2, int pos, point cen, double radius)
    {
        ...
        if (!inside)
            radius = check3 (i1, i2, z - 1,pos,cen,radius);
        return(radius); //actually radius^2
    }


    public static double check3 (int i1, int i2, int i3, int pos, point cen, double radius)
    {
        //System.out.println ("Check 3:   i1 = " + i1 + "   i2 = " + i2 + "   i3 = " + i3);
        if (angle (i1, i2, i3) >= Math.PI / 2 - d) //obtuse/right triangle
            radius = check2 (i1, i3,pos,cen,radius);
        else if (angle (i2, i1, i3) >= Math.PI / 2 - d) //obtuse/right triangle
            radius = check2 (i2, i3,pos,cen,radius);
       /* else if (angle (i1, i3, i2) >= Math.PI / 2 - d) //obtuse/right triangle
            radius = check2 (i1, i2,pos,cen,radius);*/
        else //acute triangle
        {
            boolean inside = true;
            ...
            { //step 4:
                ...
                radius = check3 (R, Q, z - 1,pos,cen,radius);
            }
        }
        return(radius);
    }
    ...
    {//Get rid of this if statement?
        if (pos > 1)    //initial segment & first segment can't intersect
        {
            boolean good = true;
    ...
    public static void recursion (int pos, double a, point cen, double radius)
    {
        double r = 0;
        point c = new point ();
        for (int k = 1 ; k < N ; k++)
        {
            ...
            if (checkI (pos))
            {
                if (sqr(P[pos+1].x-cen.x) + sqr(P[pos+1].y-cen.y) > sqr(radius) - d)
                {
                    r = Math.sqrt(check2 (0,1,pos,c,r));                    
                    System.out.println(r + "   rechecked");
                    if (r <= D/2 + d)
                    {
                        if (pos > max)
                        {
    ...
    public static void main (String [] args)
    {
        ...
        for (int z = 1; z < N/2+1; z++)
        {
            ...
            recursion (2,angles[z-1],cen,radius);
            System.out.println("\nDone: " + z);
        }
        System.out.print ("\n@ " + (max+1) + " Finished");
        System.out.println ();
