...
public class Snakes10
{
    ...
    public static boolean notAcute (int p1, int p2, int p3)
    {
        double ax = P [p1].x - P [p2].x; //-1
        double ay = P [p1].y - P [p2].y; //-1
        double bx = P [p3].x - P [p2].x; //-0.5
        double by = P [p3].y - P [p2].y; //1
        ax = (ax * bx + ay * by) / Math.sqrt (ax*ax + ay*ay) / Math.sqrt (bx*bx + by*by);
        return (ax < d);
    }
...
//test with radius = D/2 not radius = sqr... or radius = midpoint...
    public static point check3 (int i1, int i2, int i3, int pos)
    {
        point cen = new point ();
        //System.out.println ("Check 3:   i1 = " + i1 + "   i2 = " + i2 + "   i3 = " + i3);
        if (notAcute (i1, i2, i3)) //obtuse/right triangle
            cen = check2 (i1, i3, pos);
        else if (notAcute (i2, i1, i3)) //obtuse/right triangle
    ...
    public static void recursion (int pos, double a, point cen)
    {
        for (int k = 1 ; k < N ; k++)
        {
            ...
            {   //if the new point is not within the previous circle:
                ...
                {
                    point c = new point (check2 (0, 1, pos));
                    if (radius <= D*D / 4 + d)
                    {
                        if (pos > max)
                        {
...
/*
    ...
    Total Score: 23.4297 / 27 Ranking: 28   Average: 0.8678
    Scores:
    5,3     1           17      ==> 17
    5,4     1           33      ==> 33
    5,5     0.9298      53      ==> 57
