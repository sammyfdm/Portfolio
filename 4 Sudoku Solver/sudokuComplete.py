...
def findSet(sudoku):
	for br in xrange(3):
		for bc in xrange(3):
			for r in xrange(3):
				for c in xrange(3):
					if len(sudoku[br][bc][r][c]) > 1:
						#print "findset:", br, bc, r, c, sudoku[br][bc][r][c]
						return (br,bc,r,c,sudoku[br][bc][r][c])
"""
def diagCheck(s):
	u = []
	b = 0
	...
	while b < 3:
		while rc < 3:
			if 
			u.append(s[b][b][rc][rc])
"""	

def recursiveCheck(sudoku,u):
	check(sudoku,u)
	if count2(sudoku) == 0:
		return 0
	else:
		print "Recurse Call"
		(br,bc,r,c,S) = findSet(sudoku)
		for x in S:
			stemp = sudoku[:]
			stemp[br][bc][r][c] = [x]
			found(stemp,u,br,bc,r,c,x)
			if recursiveCheck(stemp,u) == 0:
				sudoku = stemp[:]
				return 0
